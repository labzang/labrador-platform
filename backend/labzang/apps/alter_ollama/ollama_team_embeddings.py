...
output_file = "app/domain/v1/soccer/models/bases/team__embeddings.py"

# teams.py ?Œì¼ ?½ê¸°
teams_file = Path("app/domain/v1/soccer/models/bases/teams.py")
teams_content = teams_file.read_text(encoding="utf-8")

# ?„ë¡¬?„íŠ¸ ?‘ì„±
prompt = f"""?¤ìŒ SQLAlchemy Team ëª¨ë¸??ì°¸ê³ ?˜ì—¬ TeamEmbedding ORM ?´ë˜?¤ë? ?‘ì„±?˜ì„¸??

=== Team ëª¨ë¸ ì½”ë“œ ===
{teams_content}

=== Alembic ë§ˆì´ê·¸ë ˆ?´ì…˜ ?Œì´ë¸??¤í‚¤ë§?===
?Œì´ë¸”ëª…: team_embeddings
ì»¬ëŸ¼:
- id: BigInteger, PK, autoincrement=True, nullable=False, comment='?„ë² ???ˆì½”??ê³ ìœ  ?ë³„??
- team_id: BigInteger, FK -> teams.id, nullable=False, ondelete='CASCADE', comment='?€ ID'
...
5. relationship: team (back_populates="embeddings") ?¤ì •
6. teams.py??ì½”ë”© ?¤í??¼ê³¼ ?¼ê???? ì? (ì£¼ì„ ?•ì‹, Column ?•ì˜ ë°©ì‹ ??
7. ëª¨ë“  Column??comment ì¶”ê?
8. __tablename__ = "team_embeddings" ?¬ìš©
9. Python ì½”ë“œë§?ì¶œë ¥ (ì£¼ì„?´ë‚˜ ?¤ëª… ?†ì´ ?œìˆ˜ ì½”ë“œë§?
10. docstring?€ Team ëª¨ë¸ê³?? ì‚¬???•ì‹?¼ë¡œ ?‘ì„±
